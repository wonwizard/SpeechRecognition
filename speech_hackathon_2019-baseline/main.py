...
def get_distance(ref_labels, hyp_labels, display=False):
    ...
    for i in range(len(ref_labels)):
        ...
        if display:
            ...
            #lnw modified
            #logger.debug('%d (%0.4f)\n(%s)\n(%s)' % (i, cer, ref, hyp))
            logger.debug('get_distance %d (%0.4f)\n(%s)\n(%s)' % (i, cer, ref, hyp))

    #lnw add for print cer
    if display:
        cer = total_dist / total_length
        logger.info('\nCER:(%0.4f)\nREF:(%s)\nHYP:(%s)' % (cer, ref_strings, hyp_strings))
...
def train(model, total_batch_size, queue, criterion, optimizer, device, train_begin, train_loader_count, print_batch=5, teacher_forcing_ratio=1):
    ...
    while True:
        ...
        #lnw add, modified i do ot want display in training
        #print("eval target,y_hat:",target,y_hat)
        #dist, length = get_distance(target, y_hat, display=display)
        dist, length = get_distance(target, y_hat, display=False)
        ...
        if batch == 90 or batch % print_batch == 0:
...
def evaluate(model, dataloader, queue, criterion, device):
    ...
    with torch.no_grad():
        while True:
            ...
            # lnw modified , no random, i want to must display 2.
            display = random.randrange(0, 100) == 0
            #lnw add
            #print("eval target,y_hat:",target,y_hat)
            if levalindex < 2 : 
...
def main():
    ...
    train_batch_num, train_dataset_list, valid_dataset = split_dataset(args, wav_paths, script_paths, valid_ratio=0.03)
    ...
    for epoch in range(begin_epoch, args.max_epochs):
        ...
        train_loss, train_cer = train(model, train_batch_num, train_queue, criterion, optimizer, device, train_begin, args.workers, 450, args.teacher_forcing)
        ...
        print(epoch,"epoch End time: "+str(lepoch_end),"Duration:",str(lepoch_end - lepoch_start),"SratTime-NowTime:",str(lepoch_end - lstart_time))
