...
#lnw cv -> test
mode = 'test' # 교차 검증 모드(cv) or 테스트 모드(test)
...
# lnw train -> test
traindataset = SpeechDataset(mode='test', label_to_int=label_to_int, wav_list=wav_list, label_list=label_list)

start_time = time()


# lnw 라벨까지 있는 검증 파일 검증용으로 라벨 있는 검증 파일이 아니면 아래 필요 없고 바로 뒷부분 테스트만 진행 
tst = 'input/val.txt'
for e in range(1):
        print("validation ", e)
        ...
        print("v End time : "+str(cv_end_time))
