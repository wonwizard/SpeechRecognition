...
#lnw add ms
sliceTime = 1000
sliceFrames = int(SR*(sliceTime/1000))
...
class SpeechDataset(Dataset):
    ...
    '''
    def get_one_word_wav(self, idx):
        # lnw. idx 번째 음성 파일을 1초만큼->sliceTime/sliceFrames만큼 읽어온다. 사전 데이터 준비에서 잘라져 있어야 한다.
        #lnw add 20190420 for stereo
        #wav = librosa.load(self.wav_list[idx], sr=self.sr)[0]
        #if len(wav) < sliceFrames :
        #    wav = np.pad(wav, (0, sliceFrames - len(wav)), 'constant')
        #return wav[:sliceFrames]
        wav = librosa.load(self.wav_list[idx], sr=self.sr, mono=False)[0]
        # lnw 20190430 add wav.flatten('F')   
        wav = wav.ravel() # stereo 2array-> 1array  [1 2],[3 4]->[1,2,3,4]
        #wav = wav.flatten('F') # stereo 2array-> 1array [1 2],[3 4]->[1,3,2,4]
        return wav
    ...
    def __getitem__(self, idx):
        ...
        if idx < len(self.wav_list):
            ...
            #lnw 20190430 add log_phase_spec, 20190503 add rms
            #wav_numpy = preprocess_mel(self.get_one_word_wav(idx))
            #wav_numpy = log_phase_spec(self.get_one_word_wav(idx))
            wav_numpy = rms_spec(self.get_one_word_wav(idx))
...
def preprocess_mel(data, n_mels=40):
    # lnw fmin=20 -> fmin=60 fmax=4000 -> fmax=2000
    #spectrogram = librosa.feature.melspectrogram(data, sr=SR, n_mels=n_mels, hop_length=160, n_fft=480, fmin=20, fmax=4000)
    spectrogram = librosa.feature.melspectrogram(data, sr=SR, n_mels=n_mels, hop_length=160, n_fft=480, fmin=60, fmax=2000)
    spectrogram = librosa.power_to_db(spectrogram)
    spectrogram = spectrogram.astype(np.float32)
    return spectrogram

# lnw 20190430 log phase spectrogram 
def log_phase_spec(data):
    #magni, phase = librosa.magphase(librosa.stft(data))
    magni, phase = librosa.magphase(librosa.stft(data,hop_length=1024)) #more short
    logp=librosa.amplitude_to_db(np.abs(phase))
    #logp=librosa.amplitude_to_db(phase, ref=np.max)
    #logp=librosa.amplitude_to_db(magni, ref=np.max)
    logp=logp[:512] #more short... del high level
    spectrogram=logp
    return spectrogram

# lnw 20190503 rms  spectrogram
def rms_spec(data):
    #magni, phase = librosa.magphase(librosa.stft(data))
    #magni, phase = librosa.magphase(librosa.stft(data,hop_length=1024)) #more short
    #rms=librosa.feature.rmse(S=magni)
    rms=librosa.feature.rmse(y=data)    
    spectrogram=rms
    return spectrogram
